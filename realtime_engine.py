...
class RealtimeEngine:
    ...
    def find_stop_id_by_name(self, stop_name: str, line_id: str) -> str:
        ...
        matches = [
            s_id
            for s_id, s_data in self.stops.items()
            if s_data["name"] == target and len(s_id) <= 3
        ]
        ...
        if not matches:
            matches = [
                s_id
                for s_id, s_data in self.stops.items()
                if target in s_data["name"] and len(s_id) <= 3
            ]
    ...
    def get_direction(self, dep_id: str, arr_id: str) -> str:
        if (
            not dep_id
            or not arr_id
            or dep_id not in self.stops
            or arr_id not in self.stops
        ):
            return "S"
    ...
    def fetch_live_arrivals(self, line_id: str, stop_id: str, direction: str) -> list:
        ...
        arrivals.sort()
        self.cache[cache_key] = {"time": time.time(), "arrivals": arrivals}
